...
def test_create_page(prism):
    ...
    folder = prism.get_folder("docs")
    page = folder.create_page("Test Page")
    ...
    assert "<!-- prism:generate:siblings -->" in content
...
def test_create_subfolder(prism):
    ...
    assert "[Parent](..)" in content  # Updated to check for relative parent link
    assert "<!-- prism:generate:subdirs -->" in content
...
def test_subfolder_parent_link(prism):
    ...
    assert "[Parent](..)" in readme_content  # Updated expectation
